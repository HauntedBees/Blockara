...
public class CharSelectController:MenuController {
	...
	private bool HandlePlayer1CursorInput() {
		...
		if(Input.GetKey(KeyCode.H) && Input.GetKey(KeyCode.K) && Input.GetKeyDown(KeyCode.N)) {
			SignalSuccess();
			string name = PD.GetPlayerSpritePathFromInt(cursor.getX());
			PD.Debug_ForceWin(name);
		}
		return pressed;
	}
