...
public class EndArcadeMatchOverlay:CharDisplayController {
	...
	public void Setup(bool won) {
		GetPersistData();
		didP1Win = won;
		playeractor = CreateActor(PD.GetPlayerSpritePath(PD.p1Char), new Vector3(-2.4f, 0f), false, true);
		playeractor.SetScale(0.4f);
		opponentactor = CreateActor(PD.GetPlayerSpritePath(PD.p2Char), new Vector3(2.4f, -0f), true, true);
